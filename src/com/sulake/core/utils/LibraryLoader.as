package com.sulake.core.utils
{
   ...
   public class LibraryLoader extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1957:uint = 1;
      
      protected static const const_1958:uint = 2;
      
      protected static const const_1956:uint = 4;
      
      protected static const const_1959:uint = 3;
      
      protected static const name_4:uint = 7;
      
      public static const const_1921:int = 5;
      
      public static const LIBRARY_LOADER_FINALIZE:String = "LIBRARY_LOADER_FINALIZE";
      
      public static const const_1314:Boolean = false;
      ...
      protected var var_667:Class;
      
      protected var var_487:uint = 0;
      
      protected var _name:String;
      
      protected var var_1106:Boolean = false;
      ...
      protected static function queue(param1:LibraryLoader) : void
      {
         ...
         {
            param1.var_43.load(param1.var_247,param1._context);
         }
      }
      ...
      protected static function throttle() : void
      {
         ...
         if(const_1314)
         {
            ...
            while(_loc2_ > -1)
            {
               _loc1_ = var_838[_loc2_] as LibraryLoader;
               if(_loc1_ && (_loc1_.ready || _loc1_.disposed))
               {
                  var_838.splice(_loc2_,1);
               }
               _loc2_--;
            }
            while(var_838.length < const_1757 && false)
            {
               _loc1_ = var_1041.shift() as LibraryLoader;
               if(!_loc1_.ready && !_loc1_.disposed)
               {
                  var_838.push(_loc1_);
                  _loc1_.var_43.load(_loc1_.var_247,_loc1_._context);
               }
            }
         }
      }
      ...
      {
         return !!this.var_1106 ? uint(this.var_1375 - this.var_2188) : uint(getTimer() - this.var_2188);
      }
      ...
      override public function dispose() : void
      {
         if(!disposed)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DISPOSE,this.var_410,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
            try
            {
               this.var_43.close();
            }
      ...
      {
         if(this.var_1107 && !_disposed)
         {
            this.var_1107 = false;
            if(!this.var_1106 && this.var_247)
            {
               this.var_1375 = -1;
      ...
      {
         if(!this.var_1106 && !_disposed && this.var_1376 > 0)
         {
            try
            {
               this.var_43.close();
            ...
            this.addRequestCounterToUrlRequest(this.var_247,const_1921 - this.var_1376);
            if(this.var_2964)
            {
            ...
            --this.var_1376;
            this.var_487 = 0;
            this.var_43.load(this.var_247,this._context);
            return true;
         }
         return false;
      }
      ...
      protected function loadEventHandler(param1:Event) : void
      {
         ...
         switch(param1.type)
         {
            ...
            case HTTPStatusEvent.HTTP_STATUS:
               ...
               this.debug("Load event STATUS " + this.var_410 + " for file \"" + this.url + "\"");
               break;
            case Event.UNLOAD:
               this.debug("Load event UNLOAD for file \"" + this.url + "\"");
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_UNLOAD,this.var_410,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case ProgressEvent.PROGRESS:
               this.debug("Load event PROGRESS for file \"" + this.url + "\"  bytes: " + this.bytesLoaded + "/" + this.bytesTotal);
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_PROGRESS,this.var_410,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case IOErrorEvent.IO_ERROR:
               this.debug("Load event IO ERROR for file \"" + this.url + "\"");
               if(!this.handleHttpStatus(this.var_410))
               {
         ...
         if(this.var_487 == LibraryLoader.const_1959)
         {
         ...
         if(this.var_487 == LibraryLoader.name_4)
         {
            if(this.prepareLibrary())
            {
               ...
               this.removeEventListeners();
               throttle();
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE,this.var_410,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
      ...
      protected function prepareLibrary() : Boolean
      {
         ...
         this.var_667 = this.getDefinition(this._name) as Class;
         if(this.var_667 == null)
         {
            if(!this.retry())
            {
               this.failure("Failed to find resource class \"" + this._name + "\" from library " + this.var_247.url + "!");
            }
            return false;
         }
         ...
         try
         {
            xmlClass = this.var_667.manifest as Class;
            if(xmlClass == null)
            {
               return false;
            }
      ...
      protected function debug(param1:String) : void
      {
         ...
         if(this._debug)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DEBUG,this.var_410,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         }
      }
      ...
      protected function failure(param1:String) : void
      {
         ...
         throttle();
         dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR,this.var_410,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         dispatchEvent(new Event(LIBRARY_LOADER_FINALIZE));
      }
