package com.sulake.core.runtime
{
   ...
   public class ComponentContext extends Component implements IContext
   {
      ...
      public final function loadFromFile(param1:URLRequest, param2:LoaderContext) : LibraryLoader
      {
         ...
         {
            if(this.var_289[_loc3_].url == param1.url)
            {
               return this.var_289[_loc3_].loadFromFile(param1);
            }
            _loc3_++;
         }
      ...
      protected function removeLibraryLoader(param1:LibraryLoader) : void
      {
         ...
         {
            if(this.var_289[_loc2_] == param1)
            {
               this.var_289.splice(_loc2_,1);
               break;
            }
            _loc2_++;
         }
         ...
         while(i < componentList.length())
         {
            ...
            if(assetCount > 0)
            {
               ...
               while(k < assetCount)
               {
                  ...
                  {
                     this.error("Manifest for component " + componentString + " contains broken asset \"" + assetName + "\"!",true,Core.const_710);
                  }
                  k++;
               }
            }
            ...
            if(componentClass == null)
            {
               this.error("Invalid component class " + componentString + "!",true,Core.const_710);
               return null;
            }
            ...
            if(component != null)
            {
               if(assetLibrary != null)
               {
                  if(component.assets != assetLibrary)
                  {
                     ...
                     this.error("Component \"" + componentString + "\" did not save provided asset library!",true,Core.const_710);
                  }
               }
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            ...
            if(this.var_289 != null)
            {
               _loc1_ = uint(this.var_289.length);
               while(_loc1_-- > 0)
               {
                  _loc2_ = this.var_289[0];
                  this.removeLibraryLoader(_loc2_);
                  _loc2_.dispose();
               }
               this.var_289 = null;
