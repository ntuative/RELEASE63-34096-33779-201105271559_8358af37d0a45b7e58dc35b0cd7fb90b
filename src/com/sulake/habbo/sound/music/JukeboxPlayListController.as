package com.sulake.habbo.sound.music
{
   ...
   public class JukeboxPlayListController implements IPlayListController
   {
      ...
      public function JukeboxPlayListController(param1:HabboSoundManagerFlash10, param2:HabboMusicController, param3:IEventDispatcher, param4:IConnection)
      {
         ...
         this.var_730 = [];
         this.var_730.push(new NowPlayingMessageEvent(this.onNowPlayingMessage));
         this.var_730.push(new JukeboxSongDisksMessageEvent(this.onJukeboxSongDisksMessage));
         this.var_730.push(new JukeboxPlayListFullMessageEvent(this.onJukeboxPlayListFullMessage));
      ...
      {
         this.var_235.stop(this.priority);
         this.var_424 = -1;
         this.var_1693 = -1;
         this.var_613 = false;
      }
      ...
      {
         if(param1 < 0 || param1 >= this.var_496.length)
         {
            return null;
         }
         return this.var_496[param1];
      }
      ...
      private function onNowPlayingMessage(param1:IMessageEvent) : void
      {
         ...
         this.var_613 = _loc3_.currentSongId != -1;
         if(_loc3_.currentSongId >= 0)
         {
            this.var_235.playSong(_loc3_.currentSongId,HabboMusicPrioritiesEnum.const_135,_loc3_.syncCount / 1000,0,1,1);
         ...
         {
            this.var_235.addSongInfoRequest(_loc3_.nextSongId);
         }
         this.var_1693 = _loc3_.currentPosition;
         this._soundManager.events.dispatchEvent(new NowPlayingEvent(NowPlayingEvent.const_103,HabboMusicPrioritiesEnum.const_135,_loc3_.currentSongId,_loc3_.currentPosition));
      }
      ...
      private function onJukeboxSongDisksMessage(param1:IMessageEvent) : void
      {
         ...
         while(_loc4_ < _loc3_.songDisks.length)
         {
            ...
            _loc7_ = this.var_235.getSongInfo(_loc5_) as SongDataEntry;
            if(_loc7_ == null)
            {
               _loc7_ = new SongDataEntry(_loc5_,-1,null,null,null);
               if(this.var_731.indexOf(_loc5_) < 0)
               {
                  this.var_731.push(_loc5_);
                  this.var_235.requestSongInfoWithoutSamples(_loc5_);
               }
            }
         ...
         if(this.var_731.length == 0)
         {
            this._events.dispatchEvent(new PlayListStatusEvent(PlayListStatusEvent.const_99));
         }
      }
      ...
      private function onJukeboxPlayListFullMessage(param1:IMessageEvent) : void
      {
         ...
         this._events.dispatchEvent(new PlayListStatusEvent(PlayListStatusEvent.const_138));
      }
      ...
      private function onSongInfoReceivedEvent(param1:SongInfoReceivedEvent) : void
      {
         ...
         {
            _loc4_ = this.var_496[_loc2_];
            if(_loc4_.id == param1.id)
            {
               _loc5_ = _loc4_.diskId;
               _loc6_ = this.var_235.getSongInfo(param1.id) as SongDataEntry;
               if(_loc6_ != null)
               {
                  _loc6_.diskId = _loc5_;
                  this.var_496[_loc2_] = _loc6_;
               }
               break;
            }
            _loc2_++;
         }
         var _loc3_:int = this.var_731.indexOf(param1.id);
         if(_loc3_ >= 0)
         {
            this.var_731.splice(_loc3_,1);
         }
         if(this.var_731.length == 0)
         {
            this._events.dispatchEvent(new PlayListStatusEvent(PlayListStatusEvent.const_99));
