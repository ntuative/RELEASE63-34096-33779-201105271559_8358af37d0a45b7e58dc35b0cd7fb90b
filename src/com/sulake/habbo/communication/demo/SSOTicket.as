package com.sulake.habbo.communication.demo
{
   ...
   public class SSOTicket extends EventDispatcher
   {
      ...
      private var var_1456:IHabboWebLogin;
      ...
      public function SSOTicket(param1:IAssetLibrary, param2:IHabboWebLogin, param3:String, param4:Boolean = true)
      {
         ...
         this.var_1455 = "http://" + param3 + "/client";
         if(!param4)
         {
            this.var_1456.init();
            this.var_1456.addEventListener(HabboWeb.const_250,this.requestClientURL);
         }
         else
         {
      ...
      {
         var _loc2_:String = this.var_1455;
         if(this._assets.hasAsset(_loc2_))
         {
            Logger.log("[CoreLocalizationManager] reload localization for url: " + this.var_1455);
         }
         var _loc3_:URLRequest = new URLRequest(this.var_1455);
         if(this._assets.hasAsset(_loc2_))
         {
      ...
      private function clientURLComplete(param1:Event = null) : void
      {
         ...
         {
            this.var_1456.requestReAuthenticate();
         }
         else
         {
            try
            {
               this.var_1773 = /\"sso.ticket\" : \"(.*?)\"/.exec(data)[1];
               this.var_1938 = /\"flash.client.url\" : \"(.*?)\"/.exec(data)[1];
               facebookData = /\"facebook.user\" : \"(.*?)\"/.exec(data);
               if(facebookData)
               {
                  this.var_2479 = Boolean(facebookData[1]);
               }
               if(this.var_1773.length > 0)
               {
