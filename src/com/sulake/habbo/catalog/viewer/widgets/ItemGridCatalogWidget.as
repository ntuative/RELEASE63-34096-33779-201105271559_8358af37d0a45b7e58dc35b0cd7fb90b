   ...
   public class ItemGridCatalogWidget extends CatalogWidget implements ICatalogWidget, IItemGrid, IDragAndDropDoneReceiver
   {
      ...
      public function ItemGridCatalogWidget(param1:IWindowContainer)
      {
         ...
         if(this._scrollBar != null)
         {
            ...
            this._scrollBar.addEventListener(WindowEvent.const_873,this.activateScrollbar);
            this._scrollBar.addEventListener(WindowEvent.const_275,this.activateScrollbar);
            this._scrollBar.addEventListener(WindowEvent.const_991,this.deActivateScrollbar);
            this._scrollBar.addEventListener(WindowEvent.const_291,this.deActivateScrollbar);
         }
      }
      ...
      protected function loadItemGridGraphics(param1:TimerEvent = null) : void
      {
         ...
         if(_loc2_ > 0)
         {
            ...
            while(_loc3_ < 3)
            {
               if(this.var_723 >= 0 && this.var_723 < _loc2_)
               {
                  _loc4_ = page.offers[this.var_723];
                  this.loadGraphics(_loc4_);
                  _loc4_.productContainer.grid = this;
               }
               ++this.var_723;
               if(this.var_723 >= _loc2_)
               {
                  this.resetTimer();
                  break;
               }
      ...
      protected function createGridItem(param1:Offer) : void
      {
         ...
         if(param1.pricingModel == Offer.const_493)
         {
            ++this.var_1669;
            (param1.productContainer as BundleProductContainer).setBundleCounter(this.var_1669);
         }
      }
