package com.sulake.habbo.room.object.visualization.furniture
{
   ...
   public class FurnitureParticleSystem
   {
      ...
      private var var_1246:Number = 1.0;
      
      private var var_645:BitmapData;
      
      private var var_797:ColorTransform;
      
      private var var_1516:ColorTransform;
      
      private var var_1905:Matrix;
      ...
      {
         super();
         this.var_1245 = new Dictionary();
         this._visualization = param1;
         this.var_797 = new ColorTransform();
         this.var_797.alphaMultiplier = 1;
         this.var_1516 = new ColorTransform();
         this.var_1905 = new Matrix();
      ...
      {
         var _loc1_:* = null;
         for each(_loc1_ in this.var_1245)
         {
            _loc1_.dispose();
         }
         this.var_1245 = null;
      ...
      public function setAnimation(param1:int) : void
      {
         ...
         this.var_104 = this.var_1245[param1];
         this.var_994 = false;
         this.updateCanvas();
      }
      ...
      private function updateCanvas() : void
      {
         ...
         if(this.var_1906 >= 0)
         {
            this.var_266 = this._visualization.getSprite(this.var_1906);
            if(this.var_266 && this.var_266.asset)
            {
               ...
               if(this.var_90 == null)
               {
                  this.var_90 = this.var_266.asset.clone();
                  if(this.var_797.alphaMultiplier != 1)
                  {
                     this.var_645 = new BitmapData(this.var_90.width,this.var_90.height,true,4278190080);
                  }
               }
               this.var_2589 = -this.var_266.offsetX;
               this.var_2588 = -this.var_266.offsetY;
               this.var_266.asset = this.var_90;
            }
            if(this.var_90)
            {
      ...
      public function getSpriteYOffset(param1:int, param2:int, param3:int) : int
      {
         ...
         {
            return this.var_104.y * this.var_1246;
         }
         return 0;
      }
      ...
      {
         if(!this.var_104 || !this.var_266)
         {
            return;
         }
         if(this.var_90 && this.var_266.asset != this.var_90)
         {
            this.var_266.asset = this.var_90;
         }
         if(this.var_994)
      ...
      public function updateAnimation() : void
      {
         ...
         var _loc2_:* = 0;
         var _loc8_:int = 0;
         if(!this.var_994 && this.var_104.hasIgnited)
         {
            this.var_994 = true;
         }
         _loc8_ = this.var_1131 * this.var_1246;
         this.var_104.update();
         if(this.var_994)
         {
            if(this.var_104.roomObjectSpriteId >= 0)
            ...
            {
               this.var_90.draw(this.var_645,this.var_1905,this.var_797,BlendMode.NORMAL,null,false);
            }
            for each(_loc11_ in this.var_104.particles)
            {
               _loc2_ = Number(_loc11_.y);
               _loc3_ = this.var_2589 + (_loc11_.x - _loc11_.z) * 10 / 10 * this.var_1246;
               _loc4_ = this.var_2588 - _loc8_ + (_loc2_ + (_loc11_.x + _loc11_.z) / 2) * 10 / 10 * this.var_1246;
               ...
               if(_loc6_)
               {
                  ...
                  if(_loc11_.fade && _loc11_.alphaMultiplier < 1)
                  {
                     ...
                     this.var_1516.alphaMultiplier = _loc11_.alphaMultiplier;
                     this.var_90.draw(_loc7_,this._translationMatrix,this.var_1516,BlendMode.NORMAL,null,false);
                  }
                  else
                  {
      ...
      public function parseData(param1:XML) : void
      {
         ...
         var _loc23_:* = null;
         var _loc24_:* = null;
         this.var_2630 = parseInt(param1.@size);
         this.var_1906 = !!param1.hasOwnProperty("@canvas_id") ? int(parseInt(param1.@canvas_id)) : -1;
         this.var_1131 = !!param1.hasOwnProperty("@offset_y") ? int(parseInt(param1.@offset_y)) : 10;
         this.var_1246 = this.var_2630 / 64;
         this._blend = !!param1.hasOwnProperty("@blend") ? Number(Number(param1.@blend)) : Number(1);
         this._blend = Math.min(this._blend,1);
         this.var_797.alphaMultiplier = this._blend;
