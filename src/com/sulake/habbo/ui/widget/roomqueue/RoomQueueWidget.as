package com.sulake.habbo.ui.widget.roomqueue
{
   ...
   public class RoomQueueWidget extends RoomWidgetBase
   {
      ...
      private function onQueueStatus(param1:RoomWidgetRoomQueueUpdateEvent) : void
      {
         ...
         this.var_2765 = param1.hasHabboClub;
         this.var_2005 = param1.isClubQueue;
         localizations.registerParameter("room.queue.position","position",this.var_1288.toString());
         localizations.registerParameter("room.queue.position.hc","position",this.var_1288.toString());
         localizations.registerParameter("room.queue.spectator.position","position",this.var_1288.toString());
         localizations.registerParameter("room.queue.spectator.position.hc","position",this.var_1288.toString());
         this.showInterface();
      }
      ...
      private function showInterface() : void
      {
         ...
         {
            switch(this.var_2006)
            {
               case RoomWidgetRoomQueueUpdateEvent.const_401:
                  _loc1_.caption = !!this.var_2005 ? "${room.queue.position.hc}" : "${room.queue.position}";
                  _loc2_.caption = "${room.queue.spectatormode}";
                  _loc3_.visible = this.var_1571;
                  break;
               case RoomWidgetRoomQueueUpdateEvent.const_554:
                  _loc1_.caption = !!this.var_2005 ? "${room.queue.spectator.position.hc}" : "${room.queue.spectator.position}";
                  _loc2_.caption = "${room.queue.back}";
                  _loc3_.visible = false;
            }
            _loc2_.visible = this.var_1571;
         }
         var _loc4_:IWindowContainer = this._window.findChildByName("club_container") as IWindowContainer;
         if(_loc4_ != null)
         {
            _loc4_.visible = !this.var_2765;
         }
      ...
      private function changeQueue(param1:WindowMouseEvent) : void
      {
         ...
         if(this.var_2006 == RoomWidgetRoomQueueUpdateEvent.const_401)
         {
            _loc2_ = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.CHANGE_TO_SPECTATOR_QUEUE);
         }
         else
         {
            _loc2_ = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_758);
         }
         messageListener.processWidgetMessage(_loc2_);
         this.removeWindow();
      }
