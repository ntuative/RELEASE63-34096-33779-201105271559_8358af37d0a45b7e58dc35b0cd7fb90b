package com.sulake.habbo.navigator
{
   ...
   public class RoomPopupCtrl extends PopupCtrl
   {
      ...
      public function RoomPopupCtrl(param1:HabboNavigator, param2:int, param3:int)
      {
         ...
         this.var_1055 = new TagRenderer(param1,hideInstantly);
      }
      ...
      override public function refreshContent(param1:IWindowContainer) : void
      {
         ...
         {
            this.var_182 = IWindowContainer(param1.findChildByName("details_container"));
         }
         this.var_182.visible = true;
         Util.hideChildren(this.var_182);
         this.refreshRoomName(this.var_182,this.var_116);
         if(this.var_116.ownerName != "" && this.var_116.ownerName != "-")
         {
         }
         this.refreshOwnerName();
         this.refreshTextWithCaption("roomctg",this.var_182,!!this.var_116.event ? this.getEventCtg(this.var_116.categoryId) : this.getRoomCtg(this.var_116.categoryId));
         this.refreshRoomDesc(this.var_182,this.var_116);
         this.refreshExtraCont();
         this.refreshInfo(this.var_182,"trading_allowed",this.var_116.allowTrading);
         this.refreshInfo(this.var_182,"doormode_doorbell",this.var_116.doorMode == 1);
         this.refreshInfo(this.var_182,"doormode_password",this.var_116.doorMode == 2);
         Util.moveChildrenToColumn(this.var_182,["roomname","roomctg_cont","roomowner_cont","roomdesc","extra_cont","doormode_doorbell","doormode_password","trading_allowed"],0,0);
         this.var_182.height = Util.getLowestPoint(this.var_182);
      }
      
      private function refreshOwnerName() : void
      {
         this.refreshTextWithCaption("roomowner",this.var_182,this.var_116.ownerName);
         var _loc1_:ITextWindow = ITextWindow(this.var_182.findChildByName("roomowner"));
         if(this.var_116.ownerName != "" && this.var_116.ownerName != "-")
         {
      ...
      {
         var _loc1_:IWindowContainer = IWindowContainer(this.var_182.findChildByName("extra_cont"));
         Util.hideChildren(_loc1_);
         this.var_1055.refreshTags(_loc1_,this.var_116.tags);
         this.refreshTextWithCaption("rating",_loc1_,"" + this.var_116.score);
         if(this.var_116.event)
         {
            this.refreshTextWithCaption("startedat",_loc1_,"" + this.var_116.eventCreationTime);
         }
         this.var_2373.refreshThumbnail(_loc1_,this.var_116.thumbnail,false);
      ...
      private function onOwnerName(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(_loc3_ != null)
         {
            navigator.mainViewCtrl.startSearch(Tabs.const_217,Tabs.const_242,_loc3_.text);
            this.hideInstantly();
         }
      }
   }
