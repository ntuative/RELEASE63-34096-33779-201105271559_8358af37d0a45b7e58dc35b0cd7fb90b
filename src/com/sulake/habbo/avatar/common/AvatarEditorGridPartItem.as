package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_1593:IWindow;
      
      private var var_859:IFigurePartSet;
      ...
      private var var_686:Rectangle;
      
      private var var_2054:int = 0;
      
      private var var_464:IAvatarRenderManager;
      
      private var var_1104:Boolean;
      
      private var var_2053:BitmapData;
      
      private const const_2262:int = 13421772;
      
      private const const_2263:int = 16777215;
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         this.var_859 = param3;
         this._window = param1;
         this.var_1593 = this._window.findChildByTag("BG_COLOR");
         this._colors = param4;
         ...
         if(param3 != null)
         {
            ...
            {
               this.var_2054 = Math.max(this.var_2054,_loc6_.colorLayerIndex);
            }
         }
         this.var_464 = this.var_72.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_706 != null && !this.var_2052)
            {
               _loc3_ = this.var_706;
            }
            else
         ...
         if(_loc2_)
         {
            if(this.var_859)
            {
               switch(this.var_859.clubLevel)
               {
                  case HabboClubLevelEnum.const_58:
                     _loc2_.visible = false;
                     break;
                  case HabboClubLevelEnum.const_45:
                     _loc2_.style = HabboIconType.const_930;
                     break;
                  case HabboClubLevelEnum.const_47:
                     _loc2_.style = HabboIconType.const_987;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this.var_1593.color = this.const_2262;
         }
         else
         {
            this.var_1593.color = this.const_2263;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_464.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_464.isFigureReady(_loc1_))
         {
            this.var_464.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_370[_loc6_] + "_" + FigureData.const_161;
               _loc4_ = this.var_464.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_686 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_2053)
               {
                  _loc9_ = this.var_72.controller.assets.getAssetByName("avatar_editor_download_icon");
                  this.var_2053 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_686.width,this.var_686.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_686.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_686.y;
               if(this.var_2052 && _loc2_.colorLayerIndex > 0)
               {
