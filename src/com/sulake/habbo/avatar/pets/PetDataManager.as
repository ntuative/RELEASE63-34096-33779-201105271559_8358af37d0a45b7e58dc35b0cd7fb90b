package com.sulake.habbo.avatar.pets
{
   ...
   public class PetDataManager implements IPetDataManager, IDisposable
   {
      ...
      public function init() : void
      {
         ...
         for each(_loc2_ in _loc1_)
         {
            if(_loc2_ != null)
            {
               ...
               if(this.var_449.getValue(_loc4_) == null)
               {
                  this.var_449.add(_loc4_,new PetData(_loc3_));
               }
            }
         }
      }
      ...
      private function notifyListeners() : void
      {
         ...
         {
            _loc1_ = this.var_541.pop() as IPetDataListener;
      ...
      {
         var _loc3_:IPetData = this.var_449.getValue(param1) as IPetData;
         if(_loc3_ == null)
         {
      ...
      public function createPetFigureString(param1:int, param2:int, param3:uint) : String
      {
         ...
         var _loc4_:PetData = this.var_449.getValue(param1);
         if(_loc4_ == null)
         {
      ...
      public function populateFigureData(param1:XML) : void
      {
         ...
         speciesKeys = this.var_449.getKeys();
         for each(key in speciesKeys)
         {
            petData = this.var_449.getValue(key);
            if(petData != null)
            {
               ...
               for each(breed in petData.breeds)
               {
                  ...
                  for each(targetSetName in targetSets)
                  {
                     ...
                     {
                        figurePart = figureTargetSet.var_610.part.(@id == petData.species).(hasOwnProperty("palettemapid") && @palettemapid == breed.patternId)[0];
                        if(figurePart == null)
                        {
                        }
      ...
      public function assetsReady(param1:int, param2:String, param3:IPetDataListener = null) : Boolean
      {
         ...
         {
            if(this.var_1537.indexOf(_loc6_) == -1)
            {
               _loc7_ = false;
               if(this.var_655.getValue(_loc6_) == null)
               {
                  ...
                  this.var_289.push(_loc8_);
                  this.var_655.add(_loc6_,_loc8_);
                  if(param3 != null && this.var_541.indexOf(param3) == -1)
                  {
                     this.var_541.push(param3);
                  }
               }
            }
         }
      ...
      private function onContentLoadError(param1:Event) : void
      {
         ...
         var _loc3_:int = this.var_289.indexOf(_loc2_);
         if(_loc3_ != -1)
         {
            this.var_289.splice(_loc3_,1);
